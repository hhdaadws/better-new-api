...
const EditChannelModal = (props) => {
  ...
  const originInputs = {
    ...
    // Claude Code 测试伪装
    claude_code_test_enabled: false,
    settings: '',
    ...
    if (success) {
      ...
      if (data.setting) {
        try {
          ...
          // Claude Code 测试伪装
          data.claude_code_test_enabled =
            parsedSettings.claude_code_test_enabled || false;
        } catch (error) {
          ...
          data.claude_code_test_enabled = false;
        }
      } else {
        data.force_format = false;
        data.thinking_to_content = false;
      ...
      if (data.settings) {
        try {
          ...
          // 读取透传全部请求头设置
          data.pass_through_headers = parsedSettings.pass_through_headers || false;
        } catch (error) {
          ...
          data.pass_through_headers = false;
        }
      } else {
  ...
  const submit = async () => {
    ...
    const channelExtraSettings = {
      ...
      // Claude Code 测试伪装
      claude_code_test_enabled: localInputs.claude_code_test_enabled || false,
    };
                  ...
                  <Card className='!rounded-2xl shadow-sm border-0 mb-6'>
                    ...
                    {[14, 33, 41].includes(inputs.type) && (
                      <>
                        ...
                        <Form.Switch
                          field='claude_code_test_enabled'
                          label={t('启用测试伪装')}
                          checkedText={t('开')}
                          uncheckedText={t('关')}
                          onChange={(value) =>
                            handleChannelSettingsChange('claude_code_test_enabled', value)
                          }
                          extraText={t(
                            '启用后，测试渠道时将使用 Claude Code 风格的请求头和请求体伪装',
                          )}
                        />
                      </>
                    )}
